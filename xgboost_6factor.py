...
def on_data(context: Context): 
    ...
    print(cal1)
    # cal[-2:-1]为本月最后一个交易日
    if now<datetime.datetime(2019,3,31):
        pass
    else:
        if now<cal1.iloc[-2]:
            ...
            price=get_reg_kdata(reg_idx=context.reg_kdata[0], target_indices=(), length=33, fill_up=True, df=True)  #获取13个月的price
            ...
            price_month1['month'] = price_month1['time'].apply(lambda x:str(x)[0:4]+str(x)[5:7])
            # price_month1['month'] = price_month1['time'].apply(int)
            ...
            factor['month'] = factor['date'].apply(lambda x: str(x)[0:4] + str(x)[5:7])
