...
"""
2016-03-31至2018-03-31训练，2018-03-31至2019-03-31测试,滚动1个月
"""
...
def on_data(context: Context): 
    ...
    # cal[-2:-1]为本月最后一个交易日
    if now<datetime.datetime(2018,4,30):   # 取2016-03-31 12个月后的日期开始交易，前11个月的数据用于训练，第13个月的为标签。如此滚动。
        pass
    else:
        if now<cal1.iloc[-2]:  # 日期小于本月交易日的倒数第二天
            ...
            price=get_reg_kdata(reg_idx=context.reg_kdata[0], target_indices=(), length=25, fill_up=True, df=True)  #获取13个月的price
            ...
            # factor的注册频率默认为日频
            factor=get_reg_factor(reg_idx=context.reg_factor[0], target_indices=(), length=500, df=True)
